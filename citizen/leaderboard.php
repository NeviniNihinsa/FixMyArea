...
/** filter by area using joins with issues */
if ($areaId > 0) {
...
{$areaWhereAssignments}
  AND a.assignment_status IN ('COMPLETED','DONE')
...
if ($areaId > 0) {
  foreach ($areas as $a) {
    if ((int)$a['area_id'] === $areaId) {
      $areaName = (string)$a['area_name'];
      break;
    }
  }
}
