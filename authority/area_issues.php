...
if ($myAreaId <= 0) {
  // Authority must be linked to an area to use this page
...
if ($q !== '') {
  $where[] = "(i.title LIKE ? OR i.issue_id LIKE ?)";
  $params[] = "%{$q}%";
  $params[] = "%{$q}%";
}
...
if ($priority !== '') {
  // assumes you added issues.priority (varchar/enum) storing low/medium/high
...
// Date range uses created_at (issue_date in low fidelity)
if ($dateFrom !== '') {
...
/**
 * Latest assignment per issue (by max assignment_id)
 */
$sqlBase = "
  SELECT
    i.issue_id,
...
<div class="container py-4 app-container">
  ...
  <div class="d-flex justify-content-between align-items-start align-items-md-center flex-column flex-md-row gap-2 mb-3">
    <div>
      ...
      <div class="text-muted small">Showing issues for: <span class="fw-semibold"><?= h($myAreaName) ?></span></div>
    </div>
      ...
      <table class="table table-dark-custom align-middle mb-0">
        <thead>
          <tr>
            ...
            <th style="width:160px;">Field Worker</th>
        ...
        <?php else: ?>
          <?php foreach ($issues as $it): ?>
            <tr>
              ...
              <td><?= h((string)($it['area_name'] ?? '')) ?></td>
              <td><?= h(ucfirst((string)($it['priority'] ?? ''))) ?></td>
